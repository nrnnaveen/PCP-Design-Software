...
export const LibraryManager: React.FC<Props> = ({
  ...
  return (
    <div
      role="dialog"
      aria-labelledby="lib-manager-title"
      aria-modal="true"
      className={`fixed inset-0 z-50 flex items-center justify-center bg-theme-modalBackdrop select-none p-3 ${isDraggingOver ? 'ring-2 ring-blue-500/50' : ''}`}
      onDragOver={handleDragOver}
      onDragLeave={handleDragLeave}
      onDrop={handleDrop}
    >
      <div className="bg-cad-panel border border-cad-border w-[1080px] max-w-full h-[680px] max-h-full rounded-sm shadow-xl flex flex-col overflow-hidden text-cad-text animate-in fade-in zoom-in-95 duration-100">
        {/* Header */}
        <div className="h-10 bg-cad-header border-b border-cad-border px-3 flex items-center justify-between">
          <div className="flex items-center space-x-2.5">
            <div className="p-1 bg-blue-500/10 text-blue-600 dark:text-blue-400 rounded-xs border border-blue-500/25">
              <Layers size={15} />
            </div>
            <div>
              <h2 id="libmanager-dialog-title" className="text-xs sm:text-sm font-semibold text-cad-textHeading flex items-center gap-2">
                ...
                <span className="text-[10px] px-1.5 py-0.2 bg-blue-500/15 text-blue-600 dark:text-blue-400 border border-blue-500/30 rounded-xs font-mono">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="px-2.5 py-1 bg-blue-600 hover:bg-blue-500 text-white rounded-xs text-xs font-medium flex items-center gap-1.5 shadow-xs transition-colors duration-fast focus-visible:outline-none"
            >
              <Upload size={12} />
            ...
            <button
              ...
              className="px-2.5 py-1 bg-cad-subpanel hover:bg-cad-surfaceHover text-cad-text border border-cad-border rounded-xs text-xs font-medium flex items-center gap-1.5 transition-colors duration-fast focus-visible:outline-none"
            >
            ...
            <button
              onClick={onClose}
              aria-label="Close"
              className="p-1 hover:bg-cad-surfaceHover rounded-xs text-cad-textMuted hover:text-cad-text ml-1 transition-colors duration-fast focus-visible:outline-none"
            >
              <X size={15} />
            </button>
            ...
            <div className="p-2 bg-cad-header border-b border-cad-border flex items-center gap-1 overflow-x-auto">
              {(['All', 'System', 'Imported', 'Project'] as const).map((cat) => (
                <button
                  ...
                  className={`px-2 py-0.5 rounded-xs text-[10px] font-mono font-medium transition-colors duration-fast ${
                    activeCategory === cat ? 'bg-blue-600 text-white font-semibold shadow-xs' : 'bg-cad-panel text-cad-textMuted hover:text-cad-text border border-cad-border'
                  }`}
                >
                  {cat}
                </button>
              ...
              {filteredLibraries.map((lib) => {
                ...
                return (
                  <div
                    ...
                    className={`p-2 rounded-xs border cursor-pointer transition-colors duration-fast ${
                      isSelected
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`text-[9px] font-mono px-1 py-0.2 rounded-xs font-medium ${
                          lib.category === 'System'
